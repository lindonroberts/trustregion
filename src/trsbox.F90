...
!
! XOPT, G, HESS, SL and SU have the same meanings as the corresponding arguments of BOBYQB.
...
use, non_intrinsic :: linalg_mod, only : inprod, issymmetric, trueloc, norm, matprod
...
real(RP) :: args(5)
real(RP) :: dred(size(g_in))
real(RP) :: gnew(size(g_in))
real(RP) :: g(size(g_in))
real(RP) :: hdred(size(g_in))
real(RP) :: hs(size(g_in))
real(RP) :: modscal
real(RP) :: s(size(g_in))
real(RP) :: sqdscr(size(g_in))
real(RP) :: ssq(size(g_in))
real(RP) :: tanbd(size(g_in))
real(RP) :: xnew(size(g_in))
...
! Scale the problem if G contains large values. Otherwise, floating point exceptions may occur.
...
! The sign of G(I) gives the sign of the change to the I-th variable that will reduce Q from its
...
xbdi = 0
xbdi(trueloc(xopt >= su .and. g <= 0)) = 1
xbdi(trueloc(xopt <= sl .and. g >= 0)) = -1
